...
function Slider() {
  ...
  const handleAdd = async () => {
    ...
    try{
       ...
       await axios.post('http://localhost:3000/api/slider',data , {
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3000/api/slider/${editIndex}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/api/slider/${id}`);
        ...
        <tbody>
          {items.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>{item.subTitle.length>120 ? item.subTitle.substring(0, 120) + "..."
                      : item.subTitle}</td>
              <td>  <img src={`http://localhost:3000/slider-image/${item.image.split('/').pop()}`} alt="Item" style={{ width: '80px' }} /></td>
              <td>
