...
function General() {
  ...
  const fetchItems = async () => {
    const response = await axios.get('http://localhost:3000/organization/general');
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`http://localhost:3000/organization/general/${editIndex}`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/organization/general/${id}`);
        ...
        <tbody>
          {items.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>  <img src={`http://localhost:3000/general/${item.image.split('/').pop()}`} alt="Item" style={{ width: '80px' }} /></td>
